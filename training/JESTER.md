...
The initial production profile uses the existing 12-block, 160-filter model, three workers with 32 concurrent games each, 256 self-play simulations, batch size 256, learning rate `3e-4`, and a 200-ply truncation cap. Twenty-five percent of starts use the Uncheck curriculum: half come from bounded forcing proofs and half are deterministic training-only positions with both an immediate losing check and a continuing alternative. Seventy-five percent are competitive standard or Uncheck-reachable openings. Competitive games use the current network 75% of the time and frozen JESTER history 25% of the time. Caps remain unknown and are masked for value training.
...
Competitive evaluation batches current JESTER against a round-robin pool containing the current champion and distinct frozen historical JESTERs under `uncheck-v1`. The promotion suite contains the standard start plus 32 fixed, reachable positions; forced single-choice losses are rejected, the side to move is balanced, and one quarter of the positions test whether the model avoids giving an immediate losing check when a continuing move exists. Each position is assigned to one opponent and played with the candidate as each color, preserving 33 paired samples without multiplying a multi-hour evaluation by the size of the opponent pool. On resume, the launcher also selects three well-spaced archives, excluding the two newest snapshots, for the frozen opponent pool.

Eight fresh, disjoint reachable positions form a rotating holdout at every evaluation. They are played against the champion and reported separately, so they reveal fixed-suite overfitting without changing promotion. Held-out tactical recognition and full conversion also remain diagnostics and do not add easy games to the promotion score. `eval.csv` and `eval_games.jsonl` retain fixed and rotating actual W/D/L, unresolved caps, paired lower bounds, color split, suite and start type, rescue attempts and successes, exposure conversions, and immediate opponent-win errors. Promotion requires fixed-suite score at least 55% and a paired conservative 95% lower bound above 50%; caps receive zero in that lower bound.
